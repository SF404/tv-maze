import { Box, Flex, HStack, Heading, Tag, Text, Button, DarkMode, Image } from '@chakra-ui/react';
            ...
            <Carousel>
                {
                    shows.map((item, index) => (
                        <Carousel.Item key={index}>
                            <Box aspectRatio={{ base: 1, md: 25 / 9 }} bg={`url(${item.show.image?.original}) no-repeat center center/cover`}>
                                <Box w={'full'} height={'full'} display={'flex'} alignItems={'center'} justifyContent={'space-around'} background={`linear-gradient(60deg, rgba(0, 0, 0, 1), rgba(0, 0, 0, 0.6),  rgba(0, 0, 0, 0.7))`}>
                                    ...
                                    <Flex gap={6} color='whitesmoke'>
                                        ...
                                        {
                                            item.show.image ? <Image src={item.show.image?.medium} rounded={10} w={'full'} loading='lazy' display={{base:'none', md:'block'}}></Image>
                                                : <Box aspectRatio={194.8 / 273.7} bg={'black'} rounded={10}></Box>

                                        }
                                        <Stack>
                                            <Heading size={['md', 'lg']}>{item.show.name}</Heading>
